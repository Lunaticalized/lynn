...
train_pos_df = train_df[train_df['sentiment'] == 0]
train_neg_df = train_df[train_df['sentiment'] == 1]
...
train_pos = sausage_data(train_pos_df)
train_neg = sausage_data(train_neg_df)

class BertExtractModel(nn.Module):
    def __init__(self):
        super(BertExtractModel, self).__init__()
        self.num_labels = 2     # pos-start, pos-end, neg-start, neg-end
    ...
    def forward(self, sentiments=None,
        ...
        logits = self.logit(sequence_output)
...
posbert = train_a_bert(train_pos)
negbert = train_a_bert(train_neg)

for model in [posbert, negbert]:
    model.eval()
    # try predict
